  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className={`${TYPE_STYLES[carta.tipo] || "bg-gray-600 border-gray-400"} border-4 rounded-xl p-6 mb-6 shadow-2xl transform-gpu backface-hidden`}
      >
        <div className="flex justify-between items-start mb-4">
          ...
          <div className="flex items-center gap-2">
            <div className={`w-3 h-3 rounded-full animate-pulse ${timeLeft < 10 ? 'bg-red-500' : 'bg-green-500'}`}></div>
            <span className="text-sm font-mono">{timeLeft}s</span>
          </div>
        </div>
        
        <div className="text-center py-4">
          <p className="text-2xl font-bold mb-4 drop-shadow-md">{carta.pergunta || carta.texto}</p>
